...
function attachmentTemplate(fileType, fileName, imgURL = null) {
  if (fileType == "file"  ) {
    ...
    return (
      ...
      <p><span class="fas fa-file-video"></span> ` +
   fileName +
   `</p>
  </div>
...
/**
 *-------------------------------------------------------------
 * Fetch id data (user) and update the view
 *-------------------------------------------------------------
 */
 ...
 *-------------------------------------------------------------
 */
 function getSharedVideos(user_id) {
  $.ajax({
    url: url + "/shared",
    method: "POST",
    data: { _token: access_token, user_id: user_id },
...
$(document).ready(function() {
  ...
  // click action for list item [user]
  $("body").on("click", ".messenger-list-item", function() {
    if (
      $(this)
        .find("tr[data-action]")
    ...
    reader.addEventListener("load", (e) => {
      $(".messenger-sendCard")
        ...
        if (!file.type.match("image.*") && !file.type.match("video.*") ) {
          ...
          sendCard.find(".attachment-preview").remove(); // older one
          sendCard.prepend(attachmentTemplate("video", file.name, e.target.result));
        }
        else {
  ...
  function attachmentValidate(file) {
    ...
    const allowedExtensions = [
      ...
      sizeLimit = 5000000000; // 500 megabyte
    ...
    if (fileSize > sizeLimit) {
      alert("Please select file size less than 500 MiB");
      return false;
    }
    return true;
  }
